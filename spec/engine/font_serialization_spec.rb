...
describe Engine::Font do
  describe ".for" do
    it "creates a font with the given path and caches it" do
      font1 = Engine::Font.for("fonts/test.ttf")
      font2 = Engine::Font.for("fonts/test.ttf")

      expect(font1).to be_a(Engine::Font)
      expect(font1.font_file_path).to eq("fonts/test.ttf")
      expect(font1.source).to eq(:game)
      expect(font1).to be(font2)
    end

    it "supports engine source" do
      font = Engine::Font.for("fonts/test.ttf", source: :engine)
  ...
  describe ".create" do
    it "creates a font with the given path" do
      font = Engine::Font.create(font_file_path: "fonts/test.ttf", source: :game)

      expect(font).to be_a(Engine::Font)
      expect(font.font_file_path).to eq("fonts/test.ttf")
    end
  end
  ...
  describe "serialization round-trip" do
    it "serializes and deserializes correctly when nested in another object" do
      ...
      expect(serialized[:font][:font_file_path]).to eq("fonts/test.ttf")
      ...
      expect(restored_wrapper.font.font_file_path).to eq("fonts/test.ttf")
